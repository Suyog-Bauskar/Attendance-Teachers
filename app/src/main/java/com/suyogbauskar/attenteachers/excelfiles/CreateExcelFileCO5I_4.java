...
public class CreateExcelFileCO5I_4 extends Service {
    ...
    private void getAllStudentsData() {
        ...
        rollNoQuery.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                sendErrorNotification(error.getMessage(), 61);
            }
        });
    }
        ...
        FirebaseDatabase.getInstance().getReference("teachers_data/" + user.getUid())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        FirebaseDatabase.getInstance().getReference("attendance/CO5I-4/" +
                                        snapshot.child("subject_code").getValue(String.class) + "/" + year)
                                .addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        if (!snapshot.exists()) {
                                            sendErrorNotification("No attendance found of CO5I-4 " + year, 62);
                                            return;
                                        }
                                        int counter = 0;
                                        ...
                                        sendNotificationOfExcelFileCreated(61);
                                        stopService(new Intent(getApplicationContext(), CreateExcelFileCO5I_4.class));
                                    }

                                    @Override
                                    public void onCancelled(@NonNull DatabaseError error) {
                                        sendErrorNotification(error.getMessage(), 63);
                                    }
                                });
                    }
                    ...
                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        sendErrorNotification(error.getMessage(), 64);
                    }
                });
    }
    ...
    private void writeExcelDataToFile(String year) {
        FirebaseDatabase.getInstance().getReference("teachers_data/" + user.getUid())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        String filename = snapshot.child("subject_name").getValue(String.class) + " CO5I-4 Attendance " + year;

                        try {
                            File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/Atten Teachers");
                            dir.mkdir();
                        } catch (Exception e) {
                            sendErrorNotification(e.getMessage(), 65);
                        }
                        ...
                        try {
                            ...
                            sendErrorNotification(e.getMessage(),66);
                        }
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        sendErrorNotification(error.getMessage(),67);
                    }
                });
    }
    ...
    private void fillStaticData() {
        ...
        for (Map.Entry<String, Map<String, Map<String, Map<String, Object>>>> entry1 : allMonthsAndChildren.entrySet()) {
            ...
            try {
                ...
                sendErrorNotification(e.getMessage(),68);
                return;
            }
